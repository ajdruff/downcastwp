...
jQuery(document).ready(function() {  //place inside a ready method or the script vars wont be available and the script will choke
    ...
    DowncastPlugins.Forms.Forms = {
        ...
        submitEvent: jQuery("form").submit(function(e) {
            ...
            if (jQuery.isEmptyObject(DowncastPlugins.Forms.Forms.response_target)) {
              DowncastPlugins.Forms.Forms.response_target = jQuery("#" + response_target_id);  
            }
           // DowncastPlugins.Forms.Forms.response_target = jQuery("#" + response_target_id);

            if (DowncastPlugins.Forms.Forms.response_target.length === 0) {
            
                response_target_id = response_target_id + '_' + form_id;
                form.before('<div '+Downcast.plugins.Forms[form_id].response_target_attributes+' id="' + response_target_id + '"><!--Ajax Response Here --></div>'
             ...
             * clear it and replace it with the ajax-loader in case an error occurs ( we dont want a success message AND an error displayed)
             */
            if (DowncastPlugins.Forms.Forms.response_target.length > 0) {
                DowncastPlugins.Forms.Forms.response_target.html(Downcast.plugins.Forms[form_id].ajax_loader_html);
        ...
        ajaxResponseHandler: function(response) {
            ...
            var response_target_id = Downcast.plugins.Forms[form_id].response_target;
            var response_target = jQuery("#" + response_target_id);


            if (response_target.length === 0) {
                response_target_id = response_target_id + '_' + form_id;
                response_target = jQuery("#" + response_target_id);

            }

            console.log('response_target_id = ' + response_target_id);


            //  var response_target = jQuery("#" + Downcast.plugins.Forms[form_id].response_target);
