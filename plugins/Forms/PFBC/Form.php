...
class Form extends Base {
    ...
    public function __construct( $id = "pfbc" ) {
        ...
        /*
         * ############## Start Downcast Custom Configuration ##################################
         */
        /*
         * Fix resource path by fixing forward slashes for Windows
         */
        $path = str_replace( '\\', '/', dirname( __FILE__ ) ) . "/Resources";
        $this->resourcesPath = substr( $path, strlen( $_SERVER[ "DOCUMENT_ROOT" ] ) );

        /*
         * Prevent Bootstrap and Jquery from loading since we're loading them via Downcast
         */

        $this->prevent = array_merge( $this->prevent, array( "bootstrap", "jQuery" ) );




        /*
         * ############## End Downcast Custom Configuration ##################################
         */
