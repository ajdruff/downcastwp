    ...
    private function _processValidationRules() {
        ...
        foreach ( $all_rules as $field_name => $rule_set ) {
            ...
            foreach ( $rule_set as $rule ) {
                ...
                $success_message = (isset($rule[ 'success_message' ]))?$rule[ 'success_message' ]:'';
                 $error_message = (isset($rule[ 'error_message' ]))?$rule[ 'error_message' ]:'';
...
/*
 * If no options have been previously set, create an empty array so merge
 * won't fail.
 */

$form_options=(isset($all_script_vars['plugins']['Forms'][$this->id()]))?$all_script_vars['plugins']['Forms'][$this->id()]:array();
